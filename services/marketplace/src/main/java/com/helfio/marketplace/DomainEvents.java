...
public class DomainEvents {
  ...
  public void append(String type, UUID aggregateId, Map<String, Object> payload) {
    try {
      jdbc.update("INSERT INTO domain_events(event_id,event_type,aggregate_id,occurred_at,version,payload) VALUES (?,?,?,?,?,?::jsonb)", UUID.randomUUID(), type, aggregateId, Timestamp.from(Instant.now()), 1, json.writeValueAsString(payload));
